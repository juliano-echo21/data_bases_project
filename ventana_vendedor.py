...
from ventana_consulta2 import ventana_consulta




class ventana_vendedor(QMainWindow):

    def __init__(self,role):
        super(ventana_vendedor,self).__init__()
        self.role = role
        self.resize(900, 650)
        #self.showMaximized()
    ...
    def setupUi(self):
        ...
        self.titulo.setGeometry(QtCore.QRect(300, 0, 500, 50))
        self.titulo.setObjectName("titulo")
        self.titulo.setText("VENTAS REALIZADAS POR VENDEDOR")
    ...
    def historial(self):
        ...
        self.table.setHorizontalHeaderLabels(['Documento','# ventas', 
                        'Total Vendido'
                        ])
        if len(l)>0:
            for i in range(len(l)):
                for j in range(len(l[0])):
                    self.table.setItem(i,j,QTableWidgetItem(str(l[i][j])))
    ...
    def crearTabla(self):
          ...
          self.table.setGeometry(QtCore.QRect(230,60,500,200))
...
def window():
    ...
    win = ventana_vendedor('jmanosalva')
    win.show()
    sys.exit(app.exec_())
